...
def start(name, proxy, user, wait_time):
    ...
    try:
        driver.get(f'http://Highline.zoom.us/wc/join/' + meetingcode)
        time.sleep(1)
...
if __name__ == '__main__':
    number = int(os.getenv('NUMBER', '10'))  # Default to 10 if not provided
    meetingcode = os.getenv('MEETINGCODE', '4242294841')  # Default meeting code
    passcode = os.getenv('PASSCODE', 'sVrr27')  # Default passcode
    sec = 4
    main()
